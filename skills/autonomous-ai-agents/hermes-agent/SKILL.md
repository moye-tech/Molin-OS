...
Installed skills: `~/.hermes/skills/` (load with `/skill <name>`).
See also: `references/integrating-external-python-packages.md` — installing companion Python projects into Hermes's stripped venv.
...
No config.yaml changes required — auto-discovered when `FEISHU_APP_ID` + `FEISHU_APP_SECRET` are set.

Requires `lark-oapi` in the Hermes venv. Hermes's venv is stripped (no `pip`); bootstrap with `python3 -m ensurepip` first.

Full setup guide: `references/feishu-setup.md`
...
### Model routing for this user (moye-tech)

- 🧠 Default model: `deepseek-v4-pro` (DeepSeek provider) — complex reasoning, code, planning
- ⚡ Simple tasks: `deepseek-v4-flash` — quick queries, simple generation
- 👁️ Vision/image tasks: Bailian/DashScope (`qwen-vl-max` or equivalent)
- 🎥 Video tasks: Bailian/DashScope HappyHorse T2V
- Auxiliary (compression, session_search, web_extract): automatically delegates when `auxiliary.*.provider: auto`

Set via `hermes config set model.default deepseek-v4-pro`.
Vision routing uses `auxiliary.vision.provider: dashscope` + `auxiliary.vision.model: qwen-vl-max`.
Always check if a task is simple enough for flash before defaulting to pro — save costs.
...
| Web dashboard | `references/hermes-web-ui.md` — community web UI (hermes-web-ui) |
