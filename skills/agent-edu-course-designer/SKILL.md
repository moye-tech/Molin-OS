# agent-edu-course-designer · 课程设计引擎
# 归属: Agent B (edu) 独占 · 版本: v1.0

## 技能身份
基于ADDIE模型 + 布鲁姆分类的教育课程设计引擎。
集成STORM深度调研生成维基百科级别的课程大纲。
...
## 不调用时机
- 只是招生/话术/学员管理 → 用对应 edu Agent 的其他技能
...
## 执行步骤（集成STORM深度调研）
Step 1: 调用 competitor-analysis 扫描同类课程竞品大纲
Step 2: 联网获取该主题最新知识体系（通过research-engine）
