...
## 🚀 Enhanced by 墨麟AI集团 — 闲鱼AI全自动运营系统

本项目在原 XianYuApis 基础上，新增以下能力：

| 模块 | 功能 | 状态 |
|------|------|------|
| **千问AI生图** | `qwen-image-2.0-pro` 生成商品封面（支持中文字体） | ✅ |
| **千问AI看图** | `qwen-vl-plus` 识别竞品图片中的文字/设计风格 | ✅ |
| **千问AI对话** | `qwen-plus` 自动回复买家咨询 | ✅ |
| **一键发布** | `xianyu_helper.py publish` — 生图+上传+发布一步完成 | ✅ |
| **WS消息监听** | `xianyu_listen.py` 实时接收买家私信 | ✅ |
| **自学习闭环** | `self_evolution.py` 每周自动复盘成功经验 | ✅ |
| **定时任务** | 消息检测（每30分钟）+ 自学习（每晚22点）+ 进化扫描（每周一） | ✅ |

### 快速使用增强功能

```bash
# 设置环境变量
export DASHSCOPE_API_KEY=sk-xxx  # 阿里百炼API Key

# 保存Cookie到文件
echo 'cookie2=xxx; _m_h5_tk=xxx; ...' > ~/.xianyu_cookies_new.txt

# 一键发布商品（生图+上传+发布）
python3.12 xianyu_helper.py publish "AI Logo设计" "专业AI生成Logo，3分钟出图" --price 199

# 看竞品图片分析
python3.12 xianyu_helper.py see "https://example.com/image.jpg" "这张图风格是什么？"

# 启动WS消息监听
python3.12 xianyu_listen.py
```

> ⚠️ **安全提醒**：Cookie和API Key通过环境变量/文件读取，不会提交到GitHub。
> 详见 `.env.dev.example` 模板。
