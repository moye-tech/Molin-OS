...
HIGH_RISK_RULES = [
    {
        ...
        "check": lambda ctx: ctx.get("is_new_credential") and ctx.get("agency") not in ("ceo",),
        "risk_level": "critical",
    },
    {
        "name": "large_transaction",
        "description": "单笔收款/退款超过阈值",
        "check": lambda ctx: ctx.get("amount", 0) > float(ctx.get("threshold", 500)),
        "risk_level": "high",
    },
    {
        "name": "mass_publish",
        "description": "发布超过指定平台数量的内容",
        "check": lambda ctx: ctx.get("publish_count", 0) > int(ctx.get("max_publish", 3)),
        "risk_level": "high",
    },
    {
        "name": "config_change",
        "description": "修改 SOP 或系统配置",
        "check": lambda ctx: ctx.get("action") in ("update_sop", "modify_config", "delete_config"),
        "risk_level": "high",
    },
    {
        "name": "data_delete",
    ...
    },
    {
        ...
        "check": lambda ctx: ctx.get("intent") in ("refund", "dispute", "complaint"),
        "risk_level": "critical",
    },
]
...
class ApprovalWorkflow:
    ...
    def __init__(self):
        ...
        self._learning_memory: List[Dict[str, Any]] = []  # 拒绝案例学习

    # ── 风险评估 ──

    def evaluate_risk(self, context: Dict[str, Any]) -> tuple[str, List[str]]:
        """
        评估任务风险等级。

        Returns:
            (risk_level, triggered_rules): 风险等级 + 触发的规则列表
        """
        triggered = []
        max_risk = "low"
            ...
            try:
                if rule["check"](context):
                    ...
                    if rule["risk_level"] == "critical":
                        max_risk = "critical"
                    elif rule["risk_level"] == "high" and max_risk != "critical":
                        max_risk = "high"
            except Exception:
                pass
    ...
    async def create_approval(
        ...
        if risk_level == "low" and not rules:
            logger.debug(f"[Approval] 低风险任务直接放行: {task_id}")
            return None  # 不需要审批
    ...
    def approve(self, approval_id: str, comment: str = "") -> Optional[ApprovalRequest]:
        """批准审批单"""
    ...
    def reject(self, approval_id: str, reason: str) -> Optional[ApprovalRequest]:
        ...
        del self._pending[approval_id]
        logger.info(f"[Approval] 已拒绝: {approval_id}, reason={reason[:50]}")
        return req

    def get_pending(self, agency: str = None) -> List[ApprovalRequest]:
        pending = list(self._pending.values())
        if agency:
            pending = [r for r in pending if r.source_agency == agency]
    ...
    def should_reject_similar(
        ...
        """检查学习记忆中是否有类似的被拒绝案例，返回拒绝原因或None"""
        # 简化的相似度检查：相同 action + 相同 agency
        for mem in self._learning_memory[-20:]:
            mem_ctx = mem.get("context", {})
            if (
                mem_ctx.get("action") == context.get("action")
                and mem_ctx.get("source_agency") == context.get("source_agency")
            ):
