...
适配自 molin-os-ultra v6.6.0 infra/security/credential_vault.py
适配: loguru → logging, 存储路径改为 ~/.hermes/vault/
"""
...
def _derive_master_key() -> bytes:
    ...
    master_password = os.getenv(
        "CREDENTIAL_MASTER_PASSWORD", "molin-ai-default-key-change-me"
    )
...
DEFAULT_ACL: Dict[str, List[str]] = {
    ...
    "dev": [],
    "ops": ["feishu", "monitoring"],
    "ceo": ["*"],  # CEO 有全部权限
}


class CredentialVault:
    """凭证保险库 — AES-256-GCM 加密 + ACL + 审计日志"""
    ...
    def store_credential(
        ...
        self._save()
        logger.info(
            f"[Vault] 凭证已存储: {record_id} (type={secret_type})"
        )
        return record_id

    def get_credential(
        self, record_id: str, requester_agency: str = ""
    ) -> Optional[str]:
        """获取解密后的凭证（需ACL检查）"""
        ...
        if not self._check_acl(requester_agency, record.platform):
            self._log_audit(
                record_id, requester_agency, "DENIED", "ACL拒绝"
            )
            logger.warning(
                f"[Vault] ACL拒绝: {requester_agency} → {record.platform}"
            )
            return None
        ...
        if record.expires_at > 0 and time.time() > record.expires_at:
            self._log_audit(
                record_id, requester_agency, "EXPIRED", "凭证已过期"
            )
            ...
            result.append(
                {
                    ...
                    "is_expired": record.expires_at > 0
