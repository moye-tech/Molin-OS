...
# ── feishu-cli 集成层 ─────────────────────────────────
# 封装 feishu-cli 二进制调用，提供统一的高阶接口。
# feishu-cli v1.22.0，已通过 OAuth Device Flow 获得 User Access Token。
# 相比 FeishuCardSender（App Token 模式），feishu-cli 模式支持：
#   - 原生 interactive 卡片发送（msg_type: interactive JSON）
#   - Markdown 直接转换为飞书文档（feishu-cli doc import）
#   - 多维表格完整 CRUD（解决之前 Bitable 权限问题）
#   - 文档导出（doc export 带走内容）
# 安装: feishu-cli -> /usr/local/bin/feishu-cli
# 授权: feishu-cli auth login --domain bitable,docs,drive,im --recommend
# Token 路径: ~/.feishu-cli/token.json
...
def feishu_bitable_record_write(base_token: str, table_id: str,
                                fields: dict) -> dict:
    """向多维表格写入一条记录"""
    import subprocess, json, tempfile
